 ...
 * [3] --存储最后一个字符c a b = 0 (因为在模式传中前缀中有下标为0开始的cab)
...
public class BMDemo {
    ...
    private static int moveByGS(int j, int m, int[] suffix, boolean[] prefix) {
        ...
        if (suffix[k] != -1) { // 存在匹配的子串
            ...
            /*
             * 为什么找前缀子串
             * 因为按理来说,若是上面找不到后缀子串,则应该将字符直接移动m (因为全后缀不能匹配, 则一定是要移动很长的)
             * --------------
             * 但是可能有这种情况:
             * 主串   efabcbcbbcee
             * 模式串    cbbc
             * 因为后缀是bc 但是找不到, 若是直接移动m则会有问题
             * efabcbcbbcee     efabcbcbbcee
             *        cbbc            cbbc
             * 应该z左边的移动过多了, 直接移动m导致出现问题.后面的子串能够匹配前缀的
             */
            for (int r = j + 2; j < m; j++) {
    ...
    private static void generateSuffix(char[] b, int m, int[] suffix, boolean[] prefix) {
        ...
        for (int i = 0; i < m - 1; i++) { // 这里只需要到m-1即可
            ...
            while (j >= 0 && b[j] == b[m - 1 - k]) {
                ...
                //简化:
                suffix[++k] = j--; // 这里的k代表后缀长度(从1开始), 这里的j代表索引(从0开始)
            }
            if (j == -1) {
