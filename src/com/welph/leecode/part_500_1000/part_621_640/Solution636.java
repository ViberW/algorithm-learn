...
public class Solution636 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(exclusiveTime(2, Arrays.asList("0:start:0", "0:start:2", "0:end:5",
    ...
    public static int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            split = log.split(":");
            function = Integer.parseInt(split[0]);
            time = Integer.parseInt(split[2]);
            if ("start".equals(split[1])) {
                if (!stack.isEmpty()) {
                    res[stack.peek()] += time - timeStack.peek();
                }
                stack.push(function);
                timeStack.push(time);
            } else {
                res[stack.pop()] += time - timeStack.pop() + 1;
                if (!stack.isEmpty()) {
                    timeStack.pop();
                    timeStack.push(time + 1);
                }
            }
        }
        return res;
