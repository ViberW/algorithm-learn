...
public class Solution458 {
    ...
    public int poorPigs1(int buckets, int minutesToDie, int minutesToTest) {
         ...
         */
        for (int i = 1; i < buckets; i++) { // combinations[i][j] i只小猪存活j只的组合数
            // C(i,k) 在存活0或者i头猪时, 组合数为1
            combinations[i][0] = 1;
            combinations[i][i] = 1;
            // C(i,k) 从i个元素中抽取k个元素的组合数
            for (int j = 1; j < i; j++) {
                ////////////////////// 选择i这个元素, 那么剩下j-1个选择/// 不选择i这个元素,从i-1选择j个元素
            ...
            // i只小猪测试iterations轮次能测试桶数 大于 buckets
    ...
    public int poorPigs2(int buckets, int minutesToDie, int minutesToTest) {
         ...
         * 那
         * (0 0 0) 代表第1轮3只猪没喝第0号桶水
         * (1 0 0) 代表第1轮第1头猪喝第1号桶水
         * (0 1 0) 代表第1轮第2头猪喝第2号桶水
         * (0 0 1) 代表第1轮第3头猪喝第3号桶水
         * ......
         * (1 1 1) 代表第1轮1,2和3头猪喝第7号桶水
         * ...
         * (2 0 0) 代表第2轮第1头猪喝第8号桶水
         * ....
         * (2 3 1) 代表第 231进制: (2*16+ 3*4 + 1*1) = 45 桶水
         * ===========>第2轮第1头猪喝, 第3轮第2头猪喝, 第1轮第3头猪喝
         * ...
         * (i j k) 代表第i轮第1头猪 j轮第2头猪 k轮第3头猪 喝第N桶水
         * 
         * 所以 一共是: (k+1) ^ n 桶水
         * --------------------------------------------
         * 当然了 我们可以这样思考
